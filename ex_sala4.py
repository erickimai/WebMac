...
def adicionar_usuario(usuario: Usuario):
    ...
    return JSONResponse({"mensagem": "Usuário adicionado", "usuario": usuario.model_dump()})


@app.get("/users")
def obter_usuarios(index: Optional[int] = Query(default=None)):
    if index is None:
        return JSONResponse({"usuarios": usuarios, "total": len(usuarios)})

    if index < 0 or index >= len(usuarios):
        return JSONResponse({"erro": f"Índice {index} fora do intervalo. Total de usuários: {len(usuarios)}"}, status_code=404)

    return JSONResponse({"index": index, "usuario": usuarios[index]})
...
def apagar_usuarios():
    ...
    usuarios.clear()
    return JSONResponse({"mensagem": f"{quantidade} usuário(s) removido(s). Lista limpa."})
